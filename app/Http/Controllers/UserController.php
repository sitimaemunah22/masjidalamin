        ...
        if ($request->ajax()) {
            ...
            return DataTables::of($log)
                ...
                ->editColumn('opsi', function($query){
                    $edit_query = route('admin.user.edit', $query->id_user);
                    $delete_query = route('admin.user.delete',['id'=>$query->id_user]);

                    $cek = (auth()->user()->role == 'masyarakat') ? 'd-none' : '';
                    $cek_2 = (auth()->user()->role != 'admin') ? 'd-none' : '';

                    $form = '<form method="POST" class="'.$cek_2.'" action="'.$delete_query.'">'.csrf_field().'<input type="submit" class="btn btn-success btn-sm m-1" onclick="return confirm(\'Data Akan Dihapus?\')" value="Delete"></form>';
                    
                    return 
                    '<div class="d-flex"><a href="'.$edit_query.'" class="btn btn-success btn-sm m-1">Edit</a>
                    '.$form.'</div>';
                })
    ...
    public function update(Request $request){
        ...
        if ($validator->fails()) {
            ...
            if($user->id_user == auth()->user()->id_user){
                $log->keterangan = auth()->user()->name.' Mengubah data akun login';
            }
            else{
            ...
            if(auth()->user()->role == 'admin'){
                ...
                return redirect()->route('admin.user.edit',['id' => $user->id_user])->with(['message' => 'Berhasil Mengubah data.']);
            }
